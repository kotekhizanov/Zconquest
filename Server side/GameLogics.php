...
}elseif ($functionName == "SendShips" && isset($_POST['gameId']) && isset($_POST['userId']) && isset($_POST['startPlanetNomer']) && isset($_POST['endPlanetNomer']) && isset($_POST['shipsAmount']))
{
	$gameId = get_post('gameId');
	$userId = get_post('userId');
	$startPlanetNomer = get_post('startPlanetNomer');
	$endPlanetNomer = get_post('endPlanetNomer');
	$shipsAmount = get_post('shipsAmount');
	
	$sql="SELECT planets.planetNomer, planets.positionX, planets.positionY FROM planets WHERE planets.gameId = '$gameId' && (planets.planetNomer = '$startPlanetNomer' || planets.planetNomer = '$endPlanetNomer')";
	...
	{
		if($startPlanetNomer == $row['planetNomer'])
		{			
			$planet1X = $row['positionX'];
			$planet1Y = $row['positionY'];
		}
		elseif($endPlanetNomer == $row['planetNomer'])
		{			
			$planet2X = $row['positionX'];
			$planet2Y = $row['positionY'];
		}
	}
